...
def _extract_cell(d: dict, line: str) -> Optional[np.ndarray]:
    ...
    # 1. Dictionary Lookup (ASE / OVITO standard)
    ...
    # Regex updated to include the closing bracket ']' for nested list formats.
...
def read_lammpstrj(
    ...
    for fi in frame_ids:
        ...
        if 'element' in col_idx:
            sym_c, use_type_map = col_idx['element'], False
        elif 'type' in col_idx:
            sym_c, use_type_map = col_idx['type'], True
        else: raise ValueError(f"No element/type info in frame {fi}.")

        if 'xs' in col_idx: cx, cy, cz, style = col_idx['xs'], col_idx['ys'], col_idx['zs'], 'scaled'
        elif 'xu' in col_idx: cx, cy, cz, style = col_idx['xu'], col_idx['yu'], col_idx['zu'], 'cartesian'
        else: cx, cy, cz, style = col_idx['x'], col_idx['y'], col_idx['z'], 'cartesian'

        id_c = col_idx.get('id')
        ids, symbs, coords = [], [], []

        for ln in raw[s + 9: s + 9 + natoms]:
            parts = ln.split()
            if id_c is not None: ids.append(int(parts[id_c]))
            symbs.append(parts[sym_c] if not use_type_map else type_map[int(parts[sym_c])])
            coords.append([float(parts[cx]), float(parts[cy]), float(parts[cz])])

        arr = np.array(coords, dtype=float)
        if sort_by_id and ids:
            order = np.argsort(ids)
            arr, symbs = arr[order], [symbs[j] for j in order]

        positions = arr @ cell_arr if style == 'scaled' else arr - origin
        frame_cell = _fallback_cell if _fallback_cell else dc_cell(cell_arr)
        frames.append(Atoms(symbs=np.array(symbs, dtype='<U2'), pos=positions, cell=frame_cell))
